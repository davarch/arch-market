        ...
        <div class="mmenu-inner pt-10">
            ...
            <nav class="flex flex-col mt-8">
                @foreach($menu as $item)
                    <a href="{{ $item['url'] }}" class="self-start py-1 text-dark hover:text-pink text-md font-bold">{{ $item['name'] }}</a>
                @endforeach
            </nav>

            <div class="flex flex-wrap items-center space-x-6 mt-8">
                <x-social-link image="icons/youtube.svg" name="YouTube" link="#" class="text-darkblue hover:text-purple"/>
                <x-social-link image="icons/telegram.svg" name="Telegram" link="#" class="text-darkblue hover:text-purple"/>
            </div>
        </div>
    </div>
</div>
